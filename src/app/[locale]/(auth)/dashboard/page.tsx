...
const DashboardIndexPage = async () => {
  ...
  try {
    if (orgId) {
      const [leadsResult, bookingsResult, aiSettingsResult] = await Promise.all([
  ...
  const channels: Array<{
    ...
    bg: string;
      ...
      { key: 'whatsapp', name: 'WhatsApp', status: 'not_connected', icon: Phone, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
      { key: 'instagram', name: 'Instagram', status: 'not_connected', icon: Instagram, color: 'text-pink-500', bg: 'bg-pink-500/10' },
      { key: 'messenger', name: 'Messenger', status: 'not_connected', icon: MessageSquare, color: 'text-blue-500', bg: 'bg-blue-500/10' },
    ];
  ...
  return (
    <div className="flex flex-col gap-8 max-w-7xl mx-auto pb-20 relative">

      {/* Background Decorative Blobs for the entire Dashboard */}
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-primary/5 blur-[120px] rounded-full pointer-events-none -z-10"></div>
      <div className="absolute top-40 left-0 w-[400px] h-[400px] bg-purple-500/5 blur-[100px] rounded-full pointer-events-none -z-10"></div>
      ...
      <div className="relative overflow-hidden flex flex-col md:flex-row justify-between items-start md:items-center gap-6 rounded-[2.5rem] p-10 shadow-2xl shadow-primary/5 border border-white/10 dark:border-white/5 bg-gradient-to-br from-card/80 to-muted/30 backdrop-blur-xl">
        ...
        <div>
          ...
          <h1 className="text-4xl md:text-5xl font-black tracking-tighter text-foreground mb-3 leading-tight">
            مرحباً بك في <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-purple-600">مركز التحكم</span> ⚡
          </h1>
          <p className="text-muted-foreground font-medium text-lg max-w-xl">
            كل ما تحتاجه لإدارة المبيعات، المحادثات، وتدريب ذكائك الاصطناعي في واجهة واحدة أنيقة.
          </p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <Link href="/dashboard/products" className="w-full md:w-auto">
            <Button variant="outline" className="w-full h-14 px-6 rounded-2xl font-black bg-background/50 backdrop-blur border-border hover:bg-muted transition-all">
              <ShoppingBag className="size-5 ms-2 text-primary" /> إضافة منتج
            </Button>
          </Link>
          <Link href="/dashboard/ai-settings" className="w-full md:w-auto">
            <Button className="w-full h-14 px-8 rounded-2xl font-black shadow-xl shadow-primary/30 hover:scale-105 active:scale-95 transition-all text-lg">
              <Bot className="size-5 ms-2" /> إعداد البوت
            </Button>
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          { label: 'العملاء المحتملين', count: leadsCount, icon: Users, color: 'text-emerald-500', bg: 'bg-emerald-500/10', trend: null, href: '/dashboard/leads' },
          { label: 'المواعيد / الحجوزات', count: bookingsCount, icon: Calendar, color: 'text-amber-500', bg: 'bg-amber-500/10', trend: null, href: '/dashboard/bookings' },
        ].map((metric, i) => {
          const CardContent = (
            <div className={`group relative bg-card/60 backdrop-blur-md border border-white/20 dark:border-white/5 p-6 rounded-[2rem] shadow-lg shadow-gray-200/20 dark:shadow-none hover:-translate-y-1 hover:shadow-xl transition-all flex flex-col justify-between overflow-hidden h-full ${metric.href ? 'cursor-pointer hover:border-primary/40' : ''}`}>
              ...
              <metric.icon className={`absolute -left-4 -bottom-4 size-32 opacity-5 group-hover:opacity-10 transition-opacity ${metric.color} -rotate-12`} />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        {/* AI Control Center (Left/Large Col) */}
          ...
          <div className="relative z-10">
            <div className="flex justify-between items-start mb-10">
              <div>
                ...
                <p className="text-slate-400 font-medium text-lg leading-relaxed max-w-lg">
                  روبوتك المخصص المتصل بعملك، مصمم ليقوم بالرد التلقائي وإغلاق الصفقات ببراعة.
                </p>
              </div>
              ...
              <div className="flex flex-col items-end gap-3">
                {isAiActive ? (
                  <div className="flex items-center gap-2 bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 px-5 py-2 rounded-2xl text-sm font-black tracking-widest uppercase shadow-lg shadow-emerald-500/10">
                    <div className="size-2.5 rounded-full bg-emerald-400 animate-pulse"></div> روبوتك في الخدمة
                  </div>
                ) : (
                  <div className="flex items-center gap-2 bg-rose-500/20 border border-rose-500/30 text-rose-400 px-5 py-2 rounded-2xl text-sm font-black tracking-widest uppercase shadow-lg shadow-rose-500/10">
                    <div className="size-2.5 rounded-full bg-rose-500 animate-pulse"></div> يرجى تفعيل الروبوت
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6 mb-12">
              <div className="bg-slate-800/40 border border-slate-700/50 rounded-3xl p-6 backdrop-blur-md">
                <div className="text-slate-400 text-xs font-black uppercase tracking-wider mb-2">محادثات مدارة بالكامل</div>
                <div className="text-4xl font-black text-white">{isAiActive ? leadsCount * 4 + 5 : 0}</div>
              </div>
              <div className="bg-slate-800/40 border border-slate-700/50 rounded-3xl p-6 backdrop-blur-md">
                <div className="text-slate-400 text-xs font-black uppercase tracking-wider mb-2">متوسط سرعة الرد</div>
            ...
            <Link href="/dashboard/ai-settings">
              <Button className={`w-full sm:w-auto h-16 px-10 text-lg font-black rounded-[1.5rem] transition-transform active:scale-95 shadow-2xl flex items-center gap-3 ${isAiActive ? 'bg-white text-slate-900 hover:bg-slate-100' : 'bg-primary text-white hover:bg-primary/90'}`}>
                ...
                {isAiActive ? 'لوحة تدريب البوت' : 'تفعيل الذكاء الاصطناعي الآن'}
              </Button>
            </Link>
          </div>
        </div>
        ...
        <div className="bg-card/80 backdrop-blur-xl border border-white/20 dark:border-white/5 rounded-[2.5rem] shadow-xl p-8 flex flex-col">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h3 className="text-2xl font-black text-foreground">قنوات التواصل</h3>
              <p className="text-sm text-muted-foreground font-medium mt-1">حالة الربط مع المنصات الاجتماعية.</p>
            </div>
      ...
      <div className="bg-gradient-to-r from-card to-muted/30 border border-white/20 dark:border-white/5 rounded-[2.5rem] p-8 shadow-lg">
        <h3 className="text-xl font-black text-foreground mb-6 flex items-center gap-3">
          ...
          انطلق بقوة: الخطوات الأساسية
        </h3>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {[
            { label: 'بيانات النشاط', desc: 'أضف تفاصيل عملك', href: '/dashboard/business', icon: Briefcase, color: 'text-indigo-500', bg: 'bg-indigo-500/10' },
            { label: 'ربط القنوات', desc: 'واتساب، ماسنجر والمزيد', href: '/dashboard/integrations', icon: LinkIcon, color: 'text-blue-500', bg: 'bg-blue-500/10' },
            { label: 'إضافة المنتجات', desc: 'لتسهيل بيعها عبر البوت', href: '/dashboard/products', icon: ShoppingBag, color: 'text-pink-500', bg: 'bg-pink-500/10' },
            { label: 'تفعيل الذكاء', desc: 'درب بعباراتك الخاصة', href: '/dashboard/ai-settings', icon: Bot, color: 'text-amber-500', bg: 'bg-amber-500/10' },
          ].map((item, i) => (
            <Link key={i} href={item.href} className="flex relative overflow-hidden items-center p-5 rounded-[2rem] border border-border bg-card shadow-sm hover:shadow-xl hover:-translate-y-1 hover:border-primary/30 transition-all group">
              <div className="absolute top-0 left-0 w-1 h-full opacity-0 group-hover:opacity-100 transition-opacity bg-primary"></div>
              <div className="flex gap-4 items-center">
                ...
                <div className="text-start">
                  <div className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1">خطوة 0{i + 1}</div>
