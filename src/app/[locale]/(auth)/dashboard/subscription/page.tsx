...
export default async function SubscriptionPage() {
  ...
  return (
    <div className="flex flex-col gap-8 w-full max-w-7xl mx-auto pb-20">
      ...
      {/* Current Plan Banner */}
      <div className={`relative overflow-hidden rounded-[2rem] border p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-5 shadow-sm ${
        ...
        <div className="flex items-center gap-4 text-start">
          <div className={`size-12 rounded-2xl flex items-center justify-center shrink-0 ${
            currentPlanId !== PLAN_ID.FREE ? 'bg-indigo-100/80 text-indigo-600' : 'bg-primary/10 text-primary'
          }`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {plans.map((plan) => {
          ...
          return (
            <div
              key={plan.id}
              className={`relative bg-card border rounded-[2rem] shadow-xl shadow-gray-100/50 overflow-hidden flex flex-col transition-all hover:scale-[1.02] hover:shadow-2xl ${
                plan.popular ? 'border-primary/30 ring-2 ring-primary/10' : 'border-border'
            ...
            >
              ...
              <div className={`p-8 text-start ${
                plan.color === 'indigo' ? 'bg-gradient-to-br from-indigo-50/50 to-purple-50/30' :
                  ...
                  return (
                    <div key={idx} className="flex items-center gap-3 text-start">
                      <div className={`size-8 rounded-xl flex items-center justify-center shrink-0 ${
                        plan.color === 'indigo' ? 'bg-indigo-50 text-indigo-500' :
              ...
              <div className="p-8 pt-0">
                {isCurrentPlan ? (
                  ...
                  <button className={`w-full h-12 rounded-2xl font-bold transition-all active:scale-95 flex items-center justify-center gap-2 ${
                    plan.popular
